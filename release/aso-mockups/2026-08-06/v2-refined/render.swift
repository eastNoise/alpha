...
private let outputDirectory = root + "/release/aso-mockups/2026-08-06/v2-refined"
private let v2Directory = root + "/release/aso-mockups/2026-08-06/v2"
private let rawDirectory = root + "/release/screenshots/raw/ko"
...
private func renderBrandPanorama(background: NSImage, icon: NSImage) throws -> NSBitmapImageRep {
    ...
    let headlines = [
        Headline(lines: [("90일로", white)], supportingText: "자기통제는 의욕이 아니라 반복으로 시작된다."),
        Headline(lines: [("네 인생을", white)], supportingText: "오늘을 끝내는 기준을 매일 쌓아라."),
        Headline(lines: [("다시 세워라.", red)], supportingText: "BASIC · STANDARD · HARD, 90일의 과정."),
    ]
...
private func renderProductPanorama(
    ...
    let headlines = [
        Headline(lines: [("오늘을", white), ("끝내라.", red)], supportingText: "확인하고, 실행하고, 하루를 마감한다."),
        Headline(lines: [("실패까지", red), ("기록하라.", white)], supportingText: "완료한 날도 무너진 날도 기록에 남는다."),
        Headline(lines: [("기준을", white), ("높여라.", red)], supportingText: "BASIC부터 HARD까지, 30일마다 다음 단계로."),
    ]
...
do {
    ...
    try drawComparison()
    print("Rendered V2 refined assets to \(outputDirectory)")
} catch {
