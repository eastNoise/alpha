...
private struct HeadlineConfig: Decodable {
    let lines: [String]
    let redLines: [Int]
...
private struct RenderConfig: Decodable {
    ...
    let brand: [HeadlineConfig]
    let product: [HeadlineConfig]
}

private let config: RenderConfig = {
    guard let argument = CommandLine.arguments.dropFirst().first else {
        fatalError("Usage: swift render-localized.swift <config.json>")
    }
...
private let outputDirectory = root + "/release/aso-mockups/2026-08-06/v2-refined/" + config.locale
...
private func makeHeadline(_ source: HeadlineConfig) -> Headline {
    Headline(
        lines: source.lines.enumerated().map { index, text in
            (text, source.redLines.contains(index) ? red : white)
        },
...
private func fittedMediumFont(text: String, maximum: CGFloat, minimum: CGFloat, width: CGFloat) -> NSFont {
    var size = maximum
    while size > minimum {
    ...
    return NSFont.systemFont(ofSize: minimum, weight: .medium)
}
...
private func renderBrandPanorama(background: NSImage, icon: NSImage) throws -> NSBitmapImageRep {
    ...
    let headlines = config.brand.map(makeHeadline)
    ...
    for (index, headline) in headlines.enumerated() {
        ...
        drawText(
            ...
            font: fittedHeavyFont(text: title.0, maximum: 164, minimum: 90, width: 1156),
            color: title.1
        )
        drawText(
            headline.supportingText,
            rect: NSRect(x: offset + 86, y: 458, width: 1120, height: 66),
            font: fittedMediumFont(text: headline.supportingText, maximum: 38, minimum: 26, width: 1120),
            color: gray
        )
...
private func drawProductHeadline(_ headline: Headline, offset: CGFloat, icon: NSImage) {
    ...
    for (index, line) in headline.lines.enumerated() {
        drawText(
            ...
            font: fittedHeavyFont(text: line.0, maximum: 112, minimum: 78, width: 1156),
            color: line.1
        )
    }
    drawText(
        ...
        font: fittedMediumFont(text: headline.supportingText, maximum: 37, minimum: 25, width: 1140),
        color: gray
    )
...
private func renderProductPanorama(
    ...
    let headlines = config.product.map(makeHeadline)
...
let requiredImages = [
    ...
    outputDirectory + "/sources/02-today.jpg",
    outputDirectory + "/sources/05-records.jpg",
    outputDirectory + "/sources/03-course.jpg",
]
...
do {
    ...
    print("Rendered \(config.locale) V2 refined assets to \(outputDirectory)")
} catch {
